nbinom.fit.robmixglm <- function(x,y,offset,gh,notrials,EMTol,  calcHessian=TRUE, cores, verbose,   starting.values=NULL) {
  ...
  ll.robustnbinom <- function(p){
    ...
    ll1 <- dnbinom(y, mu=exp(lp), size=theta, log=TRUE)
    ll2 <- dnbinom(y, mu=exp(lp), size=thetaout, log=TRUE)
  ...
  thestatistic <- function(data, ...) {
    ...
    repeat {
      ...
      if (currthetaout > currtheta) {
        temp <- currthetaout
        currthetaout <- currtheta
        currtheta <- temp
      }
    ...
    return(c(currll,currxcoef,currlpoutlier,currtheta,currthetaout))   
    },
