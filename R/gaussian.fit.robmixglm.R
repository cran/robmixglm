gaussian.fit.robmixglm <- function(x,y,offset,gh,notrials,EMTol,  calcHessian=TRUE, verbose, starting.values=NULL) {
  ...
  fitonemlreg <- function(y,outliers,x=NULL,offset=NULL,fixed) {
    ...
    if (is.null(starting.values)) {
      ...
      doprefit <- paste("robust.gaussian.prefit <- lm(y~",theformula,",",
                  "offset=offset,subset=(outliers!=1),
